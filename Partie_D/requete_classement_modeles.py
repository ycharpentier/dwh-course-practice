...
merged_df = merged_df[["nommodele", "nommagasin", "annee", "idproduit"]]
...
# On separe la liste en 2
list_models_values = [elt[0] for elt in list_models]
list_models_name = [elt[1] for elt in list_models]

# On affiche le graphique
plt.bar(list_models_name, list_models_values)
plt.xticks([])
plt.title("Diagramme en barres du nombre de produits vendus par modèle")
plt.xlabel("Modèles")
plt.ylabel("Nombre de produits vendus")
plt.tight_layout()
plt.savefig(file_path + 'requete_classement_modeles.png', bbox_inches='tight')
plt.show()