...
for elt in top_5_name:
    dico_top[elt] = []
    for annee in annees:
        dico_top[elt].append(result[(result['nommodele'] == elt) & (result['annee'] == annee)][["nbtot"]].sum()[0])
...
plt.legend(loc="best")
plt.title("Evolution du nombre de produits vendus par année\npour les 5 meilleurs modèles")
plt.xlabel("Années")
plt.ylabel("Nombre de produits vendus")
plt.savefig(file_path + 'requete_evolution_meilleurs_modeles.png', bbox_inches='tight')
plt.show()